...
@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/ui-brand.md
@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/revision-loop.md
@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/gate-prompts.md
@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/agent-contracts.md
@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/gates.md
</required_reading>
...
```bash
_GSD_SHIM_NAME="gsd-tools.cjs"; _GSD_RUNTIME_ROOT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"; GSD_TOOLS="${_GSD_RUNTIME_ROOT}/get-shit-done/bin/${_GSD_SHIM_NAME}"; if [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; elif [ -f "${_GSD_RUNTIME_ROOT}/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="${_GSD_RUNTIME_ROOT}/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; elif command -v gsd-tools >/dev/null 2>&1; then GSD_TOOLS="$(command -v gsd-tools)"; gsd_run() { "$GSD_TOOLS" "$@"; }; elif [ -f "/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; else echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS and gsd-tools is not on PATH. Run: npx -y @opengsd/gsd-core@latest --claude --local" >&2; exit 1; fi
...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/skeleton-template.md`.
...
**If found:**
1. Read template: `/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/templates/VALIDATION.md`
...
UI_GATE_JS=$(for _c in "$_GSD_RT/get-shit-done/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/bin/lib/ui-safety-gate.cjs" "$_GSD_RT/.claude/bin/lib/ui-safety-gate.cjs" "/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/bin/lib/ui-safety-gate.cjs" "/Users/vs72964/Projects/gsd-benchmark/.cursor/bin/lib/ui-safety-gate.cjs"; do [ -f "$_c" ] && { echo "$_c"; break; }; done)
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  ...
  node "/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}" --phase-req-ids "$(node "/Users/vs72964/Projects/gsd-benchmark/.cursor/get-shit-done/bin/gsd-tools.cjs" query init.plan-phase "$PHASE" --pick phase_req_ids 2>/dev/null || echo TBD)"
fi
```
